...
class Lighting_products extends ADMIN_Controller
{
    ...
    {
        parent::__construct();
        $this->load->model(array('LightingProducts_model', 'Languages_model', 'Categories_model'));
    }
    ...
    public function index($page = 0)
    {
        ...
        if (isset($_GET['delete'])) {
            $this->LightingProducts_model->deleteProduct($_GET['delete']);
        ...
        $rowscount = $this->LightingProducts_model->productsCount($search_title, $category);
        $data['products'] = $this->LightingProducts_model->getproducts($this->num_rows, $page, $search_title, $orderby, $category, $brand,$designer);
        $data['links_pagination'] = pagination('admin/lighting_products', $rowscount, $this->num_rows, 3);
        $data['num_shop_art'] = $this->LightingProducts_model->numShopproducts();
        $data['languages'] = $this->Languages_model->getLanguages();
        $data['shop_categories'] = $this->Categories_model->getShopCategories(null, null, 2);
        $this->saveHistory('Go to products');
        $this->load->view('_parts/header', $head);
        $this->load->view('ecommerce/lighting_products', $data);
    ...
    public function getProductInfo($id, $noLoginCheck = false)
    {
        ...
        return $this->LightingProducts_model->getOneProduct($id);
    }
    ...
    {
        $this->login_check();
        $result = $this->LightingProducts_model->productStatusChange($_POST['id'], $_POST['to_status']);
        if ($result == true) {
            echo 1;
        } else {
            echo 0;
