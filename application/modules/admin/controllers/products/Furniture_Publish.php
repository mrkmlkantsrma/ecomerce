...
class Furniture_Publish extends ADMIN_Controller
{
    ...
    public function index($id = 0)
    {
        ...
        if ($id > 0 && $_POST == null) {
            ...
            $data['relatedProducts'] = getRelatedProducts($id);
        ...
        if (isset($_POST['submit'])) {
            ...
            $sub_category = $_POST['shop_categorie'];
            ...
            if(!empty($this->input->post("selectedProducts"))){
                ...
                if($countPro === 4 ){
                    ...
                    $relPro = array(
                        ...
                        'type'                => 'products',
                        'product_id'          => $id,
                        'related_products'    => json_encode($this->input->post("selectedProducts")),
                    );
                    $relData =  $this->furnitureProducts_model->getEditRelProducts($id);
                   
                    if($relData['product_id'] == $id){
                        $relID = $relData['id'];
                        $this->furnitureProducts_model->updateProductRelated($relPro, $relID);
                    } else
                    {
                        $this->furnitureProducts_model->insertProductRelated($relPro);
                    }
                }
                catch(Exception $e) {
            ...
            $this->furnitureProducts_model->setProduct($_POST, $id);
        ...
        $data['related_products'] = $this->furnitureProducts_model->getEditRelProducts($id);
        $data['furCat'] = $this->furnitureProducts_model->getSubCategories('7');

        $data['furniture_cat'] = $this->furnitureProducts_model->getSubCategories(7);
        if($data['RelProducts'] != ''){
