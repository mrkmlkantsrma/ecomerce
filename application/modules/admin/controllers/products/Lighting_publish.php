...
class Lighting_publish extends ADMIN_Controller
{
    ...
    public function index($id = 0)
    {
        ...
        if (isset($_POST['submit'])) {
            ...
            $MainCat = $this->LightingProducts_model->getProCategory($main_category);
            $SubCat = $this->LightingProducts_model->getProCategory($sub_category);
            ...
            if(!empty($this->input->post("selectedProducts"))){
                ...
                if($countPro === 4 ){
                    ...
                    $relData =  $this->LightingProducts_model->getEditRelProducts($id);

                    if($relData['product_id'] == $id){
                        $relID = $relData['id'];
                        $this->LightingProducts_model->updateProductRelated($relPro, $relID);
                    } else
                    {
                        $this->LightingProducts_model->insertProductRelated($relPro);
                    }
                }
                else
                {
            ...
            $this->LightingProducts_model->setProduct($_POST, $id);
