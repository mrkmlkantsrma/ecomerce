...

{
    ...
    public function singleOrder_View($id = 0)
    {
        ...
        $head['title'] = 'Administration - order';
        ...
        $data['singleOrder'] = $this->Orders_model->getOrderByJoin($id);
        
        $orderId = $data['singleOrder']; 
        
        $id = $orderId['order_id'];

        $data['orders'] = $this->Orders_model->getOrderByOrderId($id);
        
        if (isset($_GET['delete'])) {

            $this->Orders_model->deleteOrder($_GET['delete']);

            $this->session->set_flashdata('result_delete', 'Order is deleted!');

            $this->saveHistory('Delete Order - ' . $_GET['delete']);

            redirect('admin/ecommerce/orders');

        }

        $this->load->view('_parts/header', $head);

        $this->load->view('ecommerce/single_orderDetail', $data);
	...
	public function furniture($page = 0)

    {
		...
		$data['orders'] = $this->Orders_model->getFurOrders();

        $this->saveHistory('Go to Entries');

        $this->load->view('_parts/header', $head);

        $this->load->view('ecommerce/fur_orders', $data);
