<div id="products">
    ...
    ?>
    <h1><img src="<?= base_url('assets/imgs/products-img.png') ?>" class="header-img" style="margin-top:-2px;"> furniture Products</h1>
    <hr>
    <div class="row">
        <div class="col-xs-12">
                    ...
                    <table class="table table-bordered">
                        ...
                        <tbody>
                            ...
                            foreach ($products as $row) {
                                ...
                                <tr>
                                    <td>
                                        <img src="<?= $image ?>" alt="No Image" class="img-thumbnail" style="height:100px; width:150px;">
                                    </td>
                                    <td>
                                        <a href="<?= base_url('admin/furniture_publish/' . $row->id) ?>"><?= $row->title ?></a>
                                    </td>
                                    <td>
                                        <?= $row->price ?>
                                    </td>
                                    <td><?= $row->designer_id > 0 ? '<a href="?show_designer=' . $row->designer_id . '">' . $row->designer_name . '</a>' : 'No Designer' ?></td>

                                    <!-- <td><?php //echo $row->cat_name;?></td> -->
                                    <td><?= $row->brand_id > 0 ? '<a href="?show_brand=' . $row->brand_id . '">' . $row->brand_name . '</a>' : 'No Brand' ?></td>
                                    <!-- <td><? //= $row->position ?></td> -->
                                    <td>
                                        <div class="pull-right">
                                            <a href="<?= base_url('admin/furniture_publish/' . $row->id) ?>" class="btn btn-info">Edit</a>
                                            <a href="<?= base_url('admin/furniture_products?delete=' . $row->id) ?>"  class="btn btn-danger confirm-delete">Delete</a>
                                        </div>
                                    </td>
                                </tr>
                                <?php
